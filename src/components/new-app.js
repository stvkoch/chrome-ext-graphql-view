...
function graphqlRequest(request) {
  if (!(request.method === 'POST' && request.postData && request.postData.text))
    return null
  try {
    let gqRequest = JSON.parse(request.postData.text)

    console.log('grapqhql Request', gqRequest)

    if (!Array.isArray(gqRequest)) gqRequest = [gqRequest]

    if (gqRequest.some((req) => !'query' in req)) {
      console.log('skip request', gqRequest)
      return null
    }
...
function handleHarEntry(entry) {
  const operations = parseHar(entry)
  if (operations) operations.map((op) => publish(GRAPHQL_CHANNEL, op))
}
